...
export function useChatClient({ chatId, onMessageReceived, onMessageRead, onTyping }: UseChatClientParams) {
  ...
  useEffect(() => {
    ...
    // Connect to WS backend (assumes WS server runs on API_BASE origin)
    // For local dev with expo, sometimes you need to strip /api from API_BASE
    ...
    // NetInfo listener to trigger offline sync on reconnect
    const unsubscribeNetInfo = NetInfo.addEventListener(state => {
      if (state.isConnected && newSocket.connected) {
    ...
    newSocket.on('connect', () => {
      ...
      if (chatId) {
        newSocket.emit('join_group', chatId);
      }
    ...
    newSocket.on('match_found', (data) => {
      // Could trigger global push notification or context update here
