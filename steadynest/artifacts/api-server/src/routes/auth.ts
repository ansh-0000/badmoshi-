...
// SMS-bombing protection. Two limiters, both applied to the OTP route -
// whichever trips first wins. They defend against different attacks and
// neither is sufficient alone:
//
//   per-phone  stops one victim's number being bombed
//   per-IP     stops one attacker spraying codes at many different numbers
//
// This replaces a single limiter whose key was
//     typeof req.body?.phone === "string" ? req.body.phone : req.ip
// which meant that on any real request - where `phone` is always present - the
// key was the phone and the IP limit never applied to OTP requests at all. One
// attacker could therefore request the per-number maximum against unlimited
// distinct numbers. Every one of those is a billed SMS and a stranger's phone
// buzzing at 3am.
const otpPhoneLimiter = rateLimit({
  ...
  // Requests with no usable phone are left to zod validation; the IP limiter
  // below still covers them, so nothing slips through unlimited.
...
const otpIpLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  max: 10,
  // ipKeyGenerator collapses an IPv6 address to its subnet (/56 by default).
  // Keying on the raw address would let anyone with a routed IPv6 allocation
  // use a fresh source address per request and never reach the limit at all -
  // which is what express-rate-limit's ERR_ERL_KEY_GEN_IPV6 check was warning
  // about at boot.
  keyGenerator: (req) => ipKeyGenerator(req.ip ?? ""),
  // Deliberately the same wording as the per-phone limiter. A distinguishable
  // message would tell an attacker which limit they hit, and by extension
  // something about the traffic on a number that is not theirs.
  message: {
    error: "Too many verification codes requested. Please try again later.",
  },
});
...
// No fallbacks - see the note in middlewares/auth.ts. The previous
// "access_secret_dev"/"refresh_secret_dev" defaults were token-forging
// credentials sitting in a public repo. env.ts validates these at boot and
// additionally rejects the two secrets being equal, which would let a refresh
// token pass as an access token.
...
router.post("/auth/otp/request", otpIpLimiter, otpPhoneLimiter, async (req, res) => {
...
// PATCH /auth/role — set or CHANGE the caller's role, then reissue the access
// token so its `role` claim reflects the choice.
//
// This used to 409 with "Role is already set." on any second call, which made
// the choice permanent and unrecoverable: a user who tapped the wrong card
// during onboarding was locked into that half of the product forever, with no
// way back short of deleting the account. It also made the landlord side of
// the app untestable from a tenant account.
//
// Roles are not a security boundary here — a landlord and a tenant are the
// same person wearing a different hat, and many real Delhi NCR users are both
// (subletting a room in the flat they rent). The boundary that matters is
// ownership of a specific listing or lease, and that is enforced per-resource
// by deriving owner_id from this token rather than from the request. Letting
// someone flip their own hat grants them nothing they could not already reach.
//
// The confirmation step lives on the client (Profile → role switcher), because
// the thing worth confirming is "this changes what you see", which is a UI
// concern, not something the server can meaningfully gate.
