...
import { SNSkeletonListRow } from '@/components/SNSkeleton';
...
// Static placeholder rows shown while listings load.
//
// Deliberately motion-free: this device/browser reports
// prefers-reduced-motion, which suppresses spinner and shimmer animation —
// an ActivityIndicator that can't spin is indistinguishable from a frozen
// screen. A skeleton communicates "working" through an immediate *layout*
// change instead of motion, so it still reads as responsive with reduced
// motion enabled.
// Replaced by SNSkeletonListRow from the design system. The hand-rolled version
// had drifted from the spec — three flat lines instead of two plus a
// price/rating footer row, wrong line heights, and no card shadow — and it was
// deliberately motionless, which reads as a hung screen. SNSkeletonListRow
// carries the design's geometry and an animation that adapts under reduced
// motion rather than stopping (see CLAUDE.md §7.1).
...
export default function StaysScreen() {
    ...
    <>
      <View style={[styles.headerRow, { paddingTop: topInset + 14 }]}>
        {editingLocation ? (
          ...
          >
            <Text style={[styles.eyebrow, { color: colors.mutedForeground }]}>{activeTab === 'stays' ? 'Searching near' : 'Exploring near'}</Text>
      ...
      <View style={[styles.segment, { backgroundColor: colors.muted, borderColor: colors.border }]}>
        ...
        >
          <Text style={[styles.segmentText, { color: activeTab === 'stays' ? colors.primaryForeground : colors.segmentInactive }]}>
            Stays
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text style={[styles.segmentText, { color: activeTab === 'places' ? colors.primaryForeground : colors.segmentInactive }]}>
            What's around here
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {activeTab === 'places' && (
        <>
          <View style={styles.listHeaderRow}>
            ...
            <Text style={[styles.listCount, { color: colors.mutedForeground }]}>Get oriented fast</Text>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipsScroll} contentContainerStyle={styles.chipsContent}>
            {PLACE_CATEGORIES.map((c) => (
              ...
              >
                <Text style={[styles.categoryChipText, { color: placeCategory === c.id ? colors.primaryForeground : colors.mutedForeground }]}>
                  {c.label}
                </Text>
              </TouchableOpacity>
            ))}
  ...
  // One FlashList for every state. `Header` stays mounted at the same
  // position in the tree whether we're loading, empty, or populated —
  // previously the loading branch re-parented it into a container with its
  // own paddingHorizontal, so each radius change remounted the header and
  // shifted the whole map card 22px sideways. Loading and empty states now
  // go through ListEmptyComponent, which FlashList renders reliably (its
  // empty-data + ListHeaderComponent combination did not).
  const listData: any[] = loading ? [] : activeTab === 'stays' ? stays : places;
      ...
      <View style={styles.rowWrap}>
        ...
        >
          <View style={styles.placeIcon}>
            <Feather name={PLACE_ICON[item.type as FoodPlace['type']]} size={24} color="#B5842C" />
          </View>
          <View style={{ flex: 1 }}>
  ...
  const PlacesEmpty = (
    <View style={styles.empty}>
      ...
      <Text style={[styles.emptyPlacesText, { color: colors.mutedForeground }]}>
        No {placeCategory === 'all' ? 'places' : PLACE_CATEGORIES.find((c) => c.id === placeCategory)?.label.toLowerCase()} found nearby yet.
      </Text>
    </View>
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <FlashList
        ...
        contentContainerStyle={[styles.list, { paddingBottom: tabBarClearance }] as any}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  // skeletonRow / skeletonThumb / skeletonBody / skeletonLine removed with the
  // hand-rolled StayRowSkeleton. They also carried hardcoded '#FFFFFF' and
  // 'rgba(20,32,26,0.07)' literals, which is why the loading state stayed
  // light-themed on the dark palette.
  headerRow: {
    flexDirection: 'row',
