...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [trustedContacts, setTrustedContacts] = useState<string[]>(['+919876543210']);
  ...
  }, [user, currentCityId, radius, completedItems, autopayEnabled, visitedCities, theme, trustedContacts, isAuthLoading]);
  ...
  const logout = async () => {
    // Order matters. Tokens go first so that any request already in flight
    // fails auth rather than repopulating the cache we are about to clear.
    await clearTokens();

    // Cross-user data leak, not a cosmetic bug: without this the next person
    // to sign in on this device is served the previous user's Stays results,
    // chat threads and Connect profiles. The cache is persisted to
    // AsyncStorage, so it survived app restarts too. See lib/queryClient.ts.
    ...
    // unmounts every authenticated screen holding stale props.
  };
  ...
  //
  // `activeRole` used to be independent local state that nothing but the
  // Profile money card read: tapping "Landlord" in the Profile switcher
  // swapped one mock card and changed nothing else. The user's REAL role —
  // the one in the JWT claims that decides whether AuthGuard routes them to
  // (tabs) or (landlord) — was `user.role`, and there was no way to change it
  // after onboarding. So the switcher looked like a mode switch and wasn't one.
  //
